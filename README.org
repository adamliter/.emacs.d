...
There are a few other package archives to add, however.
...
Starting with Emacs 25.1, it is possible to assign a priority to a
package archive, such that packages in higher priority package archives
will be used instead of packages in lower priority package archives.
...
This creates a function for switching between =doom-one= and
=doom-one-light=, inspired by [[https://emacs.stackexchange.com/a/26981/122][this answer on Emacs.SE]].
...
This sets up =symon=, a system monitor.
...
This sets up [[https://company-mode.github.io/][=company-mode=]] for autocompletion.
...
This sets up =forge=, which provides an interface to =git= forges like
[[https://github.com][GitHub]] and [[https://gitlab.com][GitLab]].

The variable ~auth-sources~ is defined in ~auth-source.el~, which is
part of GNU Emacs. It handles storing secrets and passwords. By setting
the first member of ~auth-sources~ to =~/.authinfo.gpg=, this will
ensure that =forge= stores tokens for accessing forges in an encrypted
file.
...
*** [[https://www.emacswiki.org/emacs/CsvMode][=csv-mode=]]
...
This sets up [[http://ess.r-project.org/][ESS]]. The (as of time of writing) unreleased version 19 of
ESS involves a significant overhaul of ESS, eliminating the smart
underscore behavior (which I don't want), sets up ESS to do keybindings
in a more modern way, among other things. This is why I have it pinned
to MELPA.
...
This sets up =scratch-ext=, which provides several nice features around
the scratch buffer, including recreating the scratch buffer whenever it
is killed or saved to a file, restoring scratch buffer from previous
session, /etc./.
...
This sets up =ws-butler=, which trims trailing white space on lines that
have been touched. It also respects ~require-final-newline~.

Note that I stopped globally deleting trailing white space on save in
93123e8ab1661c157943f517a89807e97c8f82b0 because this messed with
editing patches via ~git add -p~; white space would be trimmed, nearly
always preventing the patch from applying cleanly.

The mode used for editing patches is =diff-mode=, which is derived from
=fundamental-mode=, so it should be safe to only enable =ws-butler= in
...
Set the initial major mode for the =*scratch*= buffer to =org-mode=.
...
Make the initial Emacs frame maximized, and then make any subsequent
frames have a default maximum height and width of 100 characters.
